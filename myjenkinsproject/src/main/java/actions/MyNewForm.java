...
public class MyNewForm extends JDialog{
    ...
    private JRadioButton testReleaseRadioButton;
    ...
    private void onSubmit() {
        ...
        if(gitRepoComboBox.getSelectedItem().equals("service-market-customer-android-app")) {
            if (stageRadioButton.isSelected())
                ...
                comment = "release";
            else if (testReleaseRadioButton.isSelected())
                comment = "testRelease";
        ...
        if(comment != null && !comment.isEmpty()) {
            if(comment.equals("release")) {
                int input = JOptionPane.showConfirmDialog(null,
                        "Are you confirmed that you want to create a #Release build? Create #testRelease if you want to test any feature in Release branch. This build info will go to your manager.", "Triggering Release Build?", JOptionPane.DEFAULT_OPTION);
                if (input == 0)
    ...
    private String doGetRequest(String url, String branch, String comment) {
        ...
        Request request = new Request.Builder()
                .header("Jenkins-Crumb", UserStatePersistence.getInstance().getState().getJenkinsCrumb())
                .header("Content-type","application/x-www-form-urlencoded")
                .header("Authorization", "Basic " + UserStatePersistence.getInstance().getState().getAuthorizationToken())
                .post(requestBody)
                .url(url)
                .build();
    ...
    private void fetchBranches(String command) {
        ProcessBuilder processBuilder = new ProcessBuilder(command.split(" "));
        processBuilder.redirectErrorStream(true);
        DataContext dataContext = DataManager.getInstance().getDataContext();
        Project project = (Project) dataContext.getData(DataConstants.PROJECT);
